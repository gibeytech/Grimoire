local FilesystemExecutor = require(
   "installer.filesystem_executor"
)
...
local function map_validation_error(error_message)
   ...
   if unknown_type then
      return "Type d'opération fichier inconnu: "
         .. unknown_type
   end
...
local function execution_error(
   operation,
   filesystem_result
)
   if filesystem_result
      and filesystem_result.error
   then
      return table.concat({
   ...
   return "Échec de l'opération fichier "
      .. tostring(operation.type)
end
...
function FileOperations.simulate(result)
   ...
   if result.mode == "dry-run" then
      print(
         "[FileOperations] Dry-run : "
            .. "opération préparée"
      )
   elseif result.mode == "apply-safe" then
      print(
         "[FileOperations] Apply sécurisé : "
            .. "opération préparée mais non exécutée"
      )
   else
      print(
         "[FileOperations] Simulation : mode "
...
function FileOperations.run(operation, options)
   ...
   local result = FileOperations.prepare(
      operation,
      options
   )
